"""Render annotated charts for all filled trades from yearly v2 outputs."""
...
def _candles(ax, df: pd.DataFrame) -> None:
    for i, (_, r) in enumerate(df.iterrows()):
        ...
        ax.vlines(i, r["low"], r["high"], color=color, linewidth=1.1)
...
def _build_artifacts_for_year(data_path: Path, year: int):
...
def plot_pair_year(pair: str, data_path: Path, output_root: Path, year: int) -> int:
    # CSVs live in output_v2; charts live in output_v2/trade_charts.
    base_out = output_root if (output_root / f"{pair}_{year}_setups.csv").exists() else output_root.parent
    setups_csv = base_out / f"{pair}_{year}_setups.csv"
    trades_csv = base_out / f"{pair}_{year}_trades.csv"
    ...
    if trades.empty:
        print(f"[!] No filled trades for {pair} {year}")
        return 0
    ...
    _, df5, artifacts = _build_artifacts_for_year(data_path, year)
    ...
    for i, tr in trades.iterrows():
        ...
        xs, xf, xe = x_map.get(signal_ts), x_map.get(fill_ts), x_map.get(exit_ts)
        if xs is not None:
        ...
        entry, sl, tp = float(tr["entry_price"]), float(tr["sl_price"]), float(tr["tp_price"])
        ...
        ax.legend(uh, ul, loc="lower right", fontsize=8)

        out = out_dir / f"{pair}_{signal_ts.strftime('%Y%m%d_%H%M')}_{i+1}.png"
        fig.tight_layout()
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Plot all filled trades for a full year.")
    ...
    total = 0
    total += plot_pair_year("EURUSD", data_dir / "eurusd-m1-bid-2020-01-01-2025-12-31.parquet", out, args.year)
    total += plot_pair_year("GBPJPY", data_dir / "gbpjpy-m1-bid-2020-01-01-2025-12-31.parquet", out, args.year)
